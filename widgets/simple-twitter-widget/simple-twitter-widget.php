...
/*
	Based on plugin by Matthias Siegel. Thanks Matthias!
*/

if (!class_exists('Themeists_Twitter_Widget')) :

	class Themeists_Twitter_Widget extends WP_Widget {


		function Themeists_Twitter_Widget() {
									
			// Widget settings
			$widget_ops = array('classname' => 'themeists_twitter_widget', 'description' => 'Display your latest tweets.');

			// Create the widget
			$this->WP_Widget('themeists_twitter', 'Themeists Twitter', $widget_ops);
		}
			...
			if (!file_exists($cachefile) || (file_exists($cachefile) && (filemtime($cachefile) + $interval) < time())) :
				...
				if (method_exists($feed, 'get_items')) :
					...
					foreach ($tweets as $t) :
						...
						$result .= '<span class="twitter-username">@' . $username . '</span>';
...
if (class_exists('Themeists_Twitter_Widget')) :
	...
	function loadTwitterWidget() {
		...
		register_widget('Themeists_Twitter_Widget');
	}
