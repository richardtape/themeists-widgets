...
class NS_Widget_MailChimp extends WP_Widget {
	...
	public function NS_Widget_MailChimp () {
		...
		$this->default_title = __('Newsletter');
		$this->default_subtitle = __( 'If you would like to sign up to our mailing list then simply put your e-mail in the box below.' );
		$widget_options = array('classname' => 'themeists_mailchimp', 'description' => __( "Displays a sign-up form for a MailChimp mailing list.", 'mailchimp-widget'));
		$this->WP_Widget( 'themeists_mailchimp', __('Themeists MailChimp', 'mailchimp-widget'), $widget_options);
	...
	public function add_scripts () {
		wp_enqueue_script('ns-mc-widget', get_bloginfo('wpurl') . '/wp-content/plugins/themeists-widgets/widgets/mailchimp-widget/js/mailchimp-widget-min.js', array('jquery'), false);
	}
	...
	public function form ($instance) {
		...
		if (false == $mcapi) {
			_e( 'You need to add your Mailchimp API key before using this widget. Check Settings>Mailchimp Widget', 'mailchimp-widget' );
		} else {
			...
			?>
					...
					<p>
						<label for="<?php echo $this->get_field_id('subtitle'); ?>"><?php echo  __('Subtitle :', 'mailchimp-widget'); ?></label>
						<textarea class="widefat" id="<?php echo $this->get_field_id('subtitle'); ?>" name="<?php echo $this->get_field_name('subtitle'); ?>" type="text" value=""><?php echo $subtitle; ?></textarea>
					</p>
					<p>
	...
	public function widget ($args, $instance) {
		...
		if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
			...
			echo '<div><p>' . $instance['subtitle'] . '</p></div>';
			...
			if ($this->successful_signup) {
				...
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="<?php echo $this->id_base . '_form-' . $this->number; ?>" method="post">
					...
					?>
						...
						<input placeholder="<?php echo apply_filters( 'themeists-mailchimp-placeholder', __( 'you@yourdomain.com', 'mailchimp-widget' ) ); ?>" id="<?php echo $this->id_base; ?>-email-<?php echo $this->number; ?>" type="email" name="<?php echo $this->id_base; ?>_email" />
